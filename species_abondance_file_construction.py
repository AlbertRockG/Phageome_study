...
if len(sys.argv)<=1:
    ...
    for path in working_dir.glob("**/*_nr_viral_contigs_.csv"):
        ...
        df1 = pd.read_csv(path.absolute(), sep="\t", comment='#', skiprows=1,
                          skipinitialspace=True)
        mydirname=path.parent
        df2 = df1.loc[:,["Chr", str(mydirname)+"/"+str(mydirname)+".sam"]]
        
        df3 = df2.rename(columns={"Chr": "Contigs_id",
                                  str(mydirname)+"/"\
                                  +str(mydirname)+".sam":str(mydirname)})
        df3["Contigs_id"] = df3["Contigs_id"].str.strip()
        # open the csv file output by contids_id_taxid_and_species_extractor.py 
        # (Home-made script)
        df4 = pd.read_csv(outdir+str(mydirname)+"/"+str(mydirname)+
                          "_nr_viral_contigs__taxid.csv",
                          sep = "\t", names=["Contigs_id", "Taxid", "Species"],
                          skipinitialspace=True)
        ...
        df6["Species"]=df6["Species"].str.strip()
        df7 = df6.groupby(["Species"]).sum()
        ...
        if df8.empty:
            ...
            df8 = pd.merge(df8, df7, right_on="Species", left_on= "Species",
                           how="outer")
    
df8.to_csv(outdir +"COMPIL.csv", sep="\t", na_rep="0.0", index= False)